  ...
  using Microsoft.VisualStudio.TestTools.UnitTesting;
	...
	public static class TestHelper {
		...
		public static void CheckGrammarErrors(Parser parser) {
			...
			if (errors.Count > 0)
#if NET35
				throw new Exception("Unexpected grammar contains error(s): " + errors.StringJoin("\n"));
#else
...
#if NET35
				throw new Exception("Unexpected parse error(s): " + parseTree.ParserMessages.StringJoin("\n"));
#else
		...
		public static Token ParseInput(this Parser parser, string input, bool useTerminator = true) {
			...
			if (useTerminator) {
				...
				Assert.AreEqual(g.Terminator, tree.Tokens[count - 2].Text,
					"Input terminator not found in the second token. Input: " + input);
			}
